...
export function NetworkBadge({ isOnline, compact = false, className = '' }: NetworkBadgeProps) {
    ...
    return (
        ...
        >
            <span className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-md ${isOnline ? 'bg-[#ECFDF5] text-[#2F7D75]' : 'bg-amber-50 text-amber-600'}`}>
                <Icon name={isOnline ? 'wifi' : 'wifi-off'} className="h-3.5 w-3.5" />
            </span>
            {!compact && (
                <span className={`min-w-0 truncate text-xs font-semibold uppercase tracking-wide ${isOnline ? 'text-[#2F7D75]' : 'text-amber-700'}`}>
                    {statusLabel}
                </span>
            )}
        </div>
