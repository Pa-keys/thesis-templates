    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const { data } = await supabase
                .from('patients')
                .select('firstName, middleName, lastName, bloodType')
                .eq('id', patientId)
                .single();
            ...
            <main className="flex-1 flex flex-col min-w-0 overflow-hidden md:ml-[240px] w-full">
                <header className="h-[60px] md:h-[72px] w-full bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 sticky top-0 z-30 shadow-sm shrink-0">
                    <div className="flex items-center gap-3 md:gap-4">
                        ...
                        <button
                            ...
                            className="hidden sm:flex items-center gap-1.5 text-slate-500 hover:text-slate-800 hover:bg-slate-100 px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors border border-transparent hover:border-slate-200"
                        >
                ...
                <div className="flex-1 overflow-x-hidden overflow-y-auto w-full bg-[#F8FAFC]">
                    <div className="p-4 md:p-6 lg:p-8 mx-auto w-full max-w-5xl">
                        ...
                        <button
                            onClick={() => window.location.href = '/pages/nurse.html'}
                            className="sm:hidden mb-4 w-full flex items-center justify-center gap-2 bg-white border border-slate-200 text-slate-700 px-4 py-2.5 rounded-xl font-bold text-sm hover:bg-slate-50 active:bg-slate-100 transition-colors"
                        >
                            ← Back to Dashboard
                        </button>
                        ...
                        <form onSubmit={handleSubmit} className="w-full">
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                                        {[
                                            ...
                                            <div key={f.name} className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm">
                                                <label className="block text-[0.65rem] font-bold uppercase tracking-wider text-slate-500 mb-1.5">
                                                    {f.label}
                                                </label>
                                                <input
                                                    type={f.type}
                                                    ...
                                                    className={`w-full border border-slate-200 rounded-md px-3 py-2 text-sm font-semibold text-slate-800 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all ${(f as any).readOnly ? 'bg-slate-100 text-slate-500 cursor-not-allowed' : 'bg-slate-50 focus:bg-white'}`}
                            ...
                            <div className="flex flex-col sm:flex-row items-center justify-end gap-4 mt-6 mb-12 border-t border-slate-200 pt-6">
                                <button
                                    type="button"
                                    onClick={() => window.location.href = '/pages/nurse.html'}
                                    className="w-full sm:w-auto px-6 py-3 rounded-xl font-bold text-slate-600 hover:bg-slate-200 bg-slate-100 transition-colors text-sm"
                                >
                                    Cancel
                                </button>
